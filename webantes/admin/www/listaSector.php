...
else
{
	...
	if($_GET['param1']=="cuantos")
	{
		...
		$myquery = "SELECT COUNT(*) total FROM sector WHERE sector.eliminado = '0'";
...
sector.sector ASC LIMIT ".$mysqli->real_escape_string($_GET['limit'])." OFFSET ".$mysqli->real_escape_string($_GET["offset"]);
		...
		while($fila = $resultado ->fetch_assoc())
		{
			...
			$jsondataperson["editar"] = "<a href=addSect.php?cod=$cod><button class='btn btn-primary'><i class='fa fa-pencil'></i> Editar</button</a>";
			$jsondataperson["eliminar"] = "<a href='deletesector.php?cod=$cod'><button class='btn btn-danger'><i class='fa fa-close'></i> Eliminar</button></a>";
