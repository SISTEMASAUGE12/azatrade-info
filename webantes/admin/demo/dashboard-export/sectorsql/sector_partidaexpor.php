...
FROM exportacion INNER JOIN sector ON sector.partida = exportacion.part_nandi WHERE concat(sector.tipo,sector.sector) = '".$codeid."' $querybusca AND $wherefecha ";
  }
  if($nombres2=="diferen"){
	  $sqlyear="SELECT
...
Sum(CASE extract(year from exportacion.fnum) WHEN '2021' THEN ".$nombres2." ELSE 0 END) AS a2021,
Sum(CASE extract(year from exportacion.fnum) WHEN '2022' THEN ".$nombres2." ELSE 0 END) AS a2022
FROM exportacion INNER JOIN sector ON sector.partida = exportacion.part_nandi WHERE concat(sector.tipo,sector.sector) = '".$codeid."' $querybusca AND $wherefecha GROUP BY exportacion.part_nandi ";
						  ...
						  <h4 class="box-title">Reporte Sector Evolucion Anual de Exportaciones</h4>
							...
							<div class="box-body">
								<div class="form-group">
								  <label class="form-label"><b>Sector:</b> <?=$namesector;?> <b>│ Departamento:</b> <?=$namedepa1;?> <b>│ Indicador #1:</b> <?=$combo;?> <b>- Variable #2:</b> <?=$combo2;?> <b>│</b> Fecha Numeraci&oacute;n <b>│ Periodo:</b> 2012 - 2022 </label>
								</div>
