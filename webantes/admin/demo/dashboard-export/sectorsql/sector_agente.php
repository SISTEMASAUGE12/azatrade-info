...
FROM exportacion LEFT JOIN agente ON exportacion.cadu = agente.idagente  INNER JOIN sector ON sector.partida = exportacion.part_nandi  WHERE concat(sector.tipo,sector.sector) = '".$codeid."' $querybusca AND ".$wherefecha." ";
  }
  if($nombres2=="diferen"){
	  $sqlyear="SELECT
...
Sum(CASE ".$filtrofecha." WHEN '2021' THEN ".$nombres2." ELSE 0 END) AS a2021,
Sum(CASE ".$filtrofecha." WHEN '2022' THEN ".$nombres2." ELSE 0 END) AS a2022 
FROM exportacion INNER JOIN sector ON sector.partida = exportacion.part_nandi  WHERE concat(sector.tipo,sector.sector) = '".$codeid."' $querybusca AND ".$wherefecha." GROUP BY exportacion.cage ";
