...
$result_year -> execute(); 
$typs = $result_year -> fetchAll(PDO::FETCH_OBJ); 
if($result_year -> rowCount() > 0)   { 
foreach($typs as $fila_year) {
...
Sum(CASE ".$filtrofecha." WHEN '2021' THEN ".$nombres2." ELSE 0 END) AS a2021,
Sum(CASE ".$filtrofecha." WHEN '2022' THEN ".$nombres2." ELSE 0 END) AS a2022
FROM exportacion INNER JOIN sector ON sector.partida = exportacion.part_nandi WHERE concat(sector.tipo,sector.sector) = '".$codeid."' $querybusca AND ".$wherefecha." GROUP BY substring(exportacion.ubigeo,1,2) ";
...
$kjps = $resultado_rpt -> fetchAll(PDO::FETCH_OBJ); 
if($resultado_rpt -> rowCount() > 0)   { 
foreach($kjps as $fila_rpt) {
		   $cuentaC = $cuentaC + 1;
...
$sql_depa = $conexpg -> prepare($sql_depa); 
$sql_depa -> execute(); 
$deps = $sql_depa -> fetchAll(PDO::FETCH_OBJ); 
if($sql_depa -> rowCount() > 0)   { 
foreach($deps as $fila_depa) {
