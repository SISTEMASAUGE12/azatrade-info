...
FROM exportacion LEFT JOIN aduana ON exportacion.cadu = aduana.codadu INNER JOIN sector ON sector.partida = exportacion.part_nandi WHERE concat(sector.tipo,sector.sector) = '".$codeid."' $querybusca AND ".$wherefecha." ";
  }
  if($nombres2=="diferen"){
	  $sqlyear="SELECT
...
Sum(CASE ".$filtrofecha." WHEN '2021' THEN ".$nombres2." ELSE 0 END) AS a2021,
Sum(CASE ".$filtrofecha." WHEN '2022' THEN ".$nombres2." ELSE 0 END) AS a2022 
FROM exportacion LEFT JOIN aduana ON exportacion.cadu = aduana.codadu  INNER JOIN sector ON sector.partida = exportacion.part_nandi  WHERE concat(sector.tipo,sector.sector) = '".$codeid."' $querybusca AND ".$wherefecha." GROUP BY exportacion.cadu,aduana.descripcion ";
...
$teps = $resultado_rpt -> fetchAll(PDO::FETCH_OBJ); 
if($resultado_rpt -> rowCount() > 0)   { 
foreach($teps as $fila_rpt) {
