...
FROM exportacion LEFT JOIN puerto ON exportacion.cpuedes = puerto.idcodigo INNER JOIN sector ON sector.partida = exportacion.part_nandi WHERE concat(sector.tipo,sector.sector) = '".$codeid."' $querybusca AND ".$wherefecha." ";
  }
  if($nombres2=="diferen"){
	  $sqlyear="SELECT
...
Sum(CASE ".$filtrofecha." WHEN '2021' THEN 1 ELSE 0 END) AS a2022 
FROM exportacion LEFT JOIN puerto ON exportacion.cpuedes = puerto.idcodigo INNER JOIN sector ON sector.partida = exportacion.part_nandi WHERE concat(sector.tipo,sector.sector) = '".$codeid."' $querybusca AND ".$wherefecha." ";
  }
...
COUNT(DISTINCT CASE ".$filtrofecha." WHEN '2021' THEN exportacion.".$nombres2." ELSE NULL END) AS a2022 
FROM exportacion LEFT JOIN puerto ON exportacion.cpuedes = puerto.idcodigo INNER JOIN sector ON sector.partida = exportacion.part_nandi WHERE concat(sector.tipo,sector.sector) = '".$codeid."' $querybusca AND ".$wherefecha." ";
  }
...
COUNT(DISTINCT CASE ".$filtrofecha." WHEN '2021' THEN ".$varia." ELSE NULL END) AS a2022
FROM exportacion LEFT JOIN puerto ON exportacion.cpuedes = puerto.idcodigo INNER JOIN sector ON sector.partida = exportacion.part_nandi WHERE concat(sector.tipo,sector.sector) = '".$codeid."' $querybusca AND ".$wherefecha." ";   
  }
$result_year = $conexpg -> prepare($sqlyear); 
$result_year -> execute(); 
$uups = $result_year -> fetchAll(PDO::FETCH_OBJ); 
if($result_year -> rowCount() > 0)   { 
foreach($uups as $fila_year) {
...
Sum(CASE ".$filtrofecha." WHEN '2021' THEN ".$nombres2." ELSE 0 END) AS a2021,
Sum(CASE ".$filtrofecha." WHEN '2022' THEN ".$nombres2." ELSE 0 END) AS a2022 
FROM exportacion LEFT JOIN puerto ON exportacion.cpuedes = puerto.idcodigo INNER JOIN sector ON sector.partida = exportacion.part_nandi  WHERE concat(sector.tipo,sector.sector) = '".$codeid."' $querybusca AND ".$wherefecha." GROUP BY exportacion.cpuedes,puerto.puerto ";
				  ...
				  <div class="col-lg-12 col-12">
					  <div class="box">
						<div class="box-header with-border">
						  <h4 class="box-title">Reporte Sector Evoluci&oacute;n Mensual de Exportaciones</h4>
						</div>
							<div class="box-body">
...
$uyps = $resultado_rpt -> fetchAll(PDO::FETCH_OBJ); 
if($resultado_rpt -> rowCount() > 0)   { 
foreach($uyps as $fila_rpt) {
			   $cuentaA = $cuentaA + 1;
