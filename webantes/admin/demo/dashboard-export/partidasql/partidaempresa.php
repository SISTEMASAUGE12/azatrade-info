...
$partida1 = trim($_POST["partidaemp"]);
$ubicod1 = trim($_POST["codubiemp"]);
$dato3 = trim($_POST["namedepaemp"]);
$filtro = trim($_POST["unavariaemp"]);
...
FROM exportacion where $ranfecha AND exportacion.part_nandi=".$partida1." $codi1 ";
	}
	if($filtro=="vpesnet"){
		$sqlyear="SELECT 'vfobserdol' as VALOR, 
...
FROM exportacion where $ranfecha AND exportacion.part_nandi=".$partida1." $codi1 GROUP BY diferen";
				}	
...
foreach($ramps as $fila_year) {
   ...
   SUM(CASE extract(year from exportacion.fnum) WHEN '2021' THEN exportacion.vfobserdol ELSE 0 END) AS a2021,
   SUM(CASE extract(year from exportacion.fnum) WHEN '2022' THEN exportacion.vfobserdol ELSE 0 END) AS a2022
   FROM exportacion where $ranfecha AND exportacion.part_nandi=".$partida1." $codi1 GROUP BY exportacion.dnombre";
	}
				if($filtro=="vpesnet"){
					$query1 = "SELECT exportacion.dnombre, 
   SUM(CASE extract(year from exportacion.fnum) WHEN '2012' THEN exportacion.vpesnet ELSE 0 END) AS a2012, 
...
<div class="col-lg-12 col-12">
					  <div class="box">
						<div class="box-header with-border">
						  <h4 class="box-title">Reporte Empresas Exportadoras Evoluci&oacute;n Anual de Exportaciones</h4>
						</div>
							<div class="box-body">
