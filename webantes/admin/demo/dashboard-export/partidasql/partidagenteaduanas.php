...
$partida1 = trim($_POST["partidaage"]);
$ubicod1 = trim($_POST["codubiage"]);
$dato3 = trim($_POST["namedepage"]);
$filtro = trim($_POST["unavariaage"]);
...
FROM exportacion LEFT JOIN agente ON exportacion.CAGE = agente.idagente where $ranfecha AND exportacion.part_nandi=".$partida1." $codi1 ";
	}
	if($filtro=="vpesnet"){
		$sqlyear="SELECT 'vfobserdol' as VALOR, 
...
FROM exportacion LEFT JOIN agente ON exportacion.CAGE = agente.idagente where $ranfecha AND exportacion.part_nandi=".$partida1." $codi1 GROUP BY diferen";
				}	
...
foreach($ramps as $fila_year) {
   ...
   SUM(CASE extract(year from exportacion.fnum) WHEN '2021' THEN exportacion.vfobserdol ELSE 0 END) AS a2021,
   SUM(CASE extract(year from exportacion.fnum) WHEN '2022' THEN exportacion.vfobserdol ELSE 0 END) AS a2022
   FROM exportacion LEFT JOIN agente ON exportacion.CAGE = agente.idagente where $ranfecha AND exportacion.part_nandi=".$partida1." $codi1 GROUP BY agente.agencia";
	}
				if($filtro=="vpesnet"){
					$query1 = "SELECT agente.agencia,
   SUM(CASE extract(year from exportacion.fnum) WHEN '2012' THEN exportacion.vpesnet ELSE 0 END) AS a2012, 
...
<div class="col-lg-12 col-12">
					  <div class="box">
						<div class="box-header with-border">
						  <h4 class="box-title">Reporte Agente de Aduanas Exportadoras de Exportaciones</h4>
						</div>
							<div class="box-body">
