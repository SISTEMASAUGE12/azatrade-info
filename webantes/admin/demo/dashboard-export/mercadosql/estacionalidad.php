...
$paisname = trim($_POST["namepaisC"]);
$paiscode = trim($_POST["codepaisC"]);
...
//realizamos conusulta del reporte selecionado y  actualizamos en la tabla temporal crada
 ...
 WHERE exportacion.cpaides='".$paiscode."' $queryu $ranfecha GROUP BY exportacion.fano,mes ORDER BY exportacion.fano,mes";
  }
  if($onevar=="vpesnet"){
  $query1 = "SELECT exportacion.fano, extract(MONTH from exportacion.fnum) as mes, Sum(exportacion.vpesnet) as resultado FROM exportacion WHERE exportacion.cpaides = '".$paiscode."' $queryu $ranfecha GROUP BY exportacion.fano,mes ORDER BY exportacion.fano,mes";
  }
  if($onevar=="diferen"){
  $query1 = "SELECT exportacion.fano, extract(MONTH from exportacion.fnum) as mes, Sum(exportacion.vfobserdol) / Sum(exportacion.vpesnet) as resultado FROM exportacion WHERE exportacion.cpaides = '".$paiscode."' $queryu $ranfecha GROUP BY exportacion.fano,mes ORDER BY exportacion.fano,mes";
  }
  if($onevar=="vpesbru"){
  $query1 = "SELECT exportacion.fano, extract(MONTH from exportacion.fnum) as mes, Sum(exportacion.vpesbru) as resultado FROM exportacion WHERE exportacion.cpaides = '".$paiscode."' $queryu $ranfecha GROUP BY exportacion.fano,mes ORDER BY exportacion.fano,mes";
  }
   if($onevar=="qunifis"){
  $query1 = "SELECT exportacion.fano, extract(MONTH from exportacion.fnum) as mes, Sum(exportacion.qunifis) as resultado FROM exportacion WHERE exportacion.cpaides = '".$paiscode."' $queryu $ranfecha GROUP BY exportacion.fano,mes ORDER BY exportacion.fano,mes";
  }
  if($onevar=="qunicom"){
  $query1 = "SELECT exportacion.fano, extract(MONTH from exportacion.fnum) as mes, Sum(exportacion.qunicom) as resultado FROM exportacion WHERE exportacion.cpaides = '".$paiscode."' $queryu $ranfecha GROUP BY exportacion.fano,mes ORDER BY exportacion.fano,mes";
  }
  if($onevar=="part_nandi"){
	  $query1 = "SELECT exportacion.fano, extract(MONTH from exportacion.fnum) as mes,COUNT(DISTINCT exportacion.part_nandi) as resultado FROM exportacion WHERE exportacion.cpaides = '".$paiscode."' $queryu $ranfecha GROUP BY exportacion.fano,mes ORDER BY exportacion.fano,mes";
  }
  if($onevar=="total"){
  $query1 = "SELECT exportacion.fano, extract(MONTH from exportacion.fnum) as mes, count(*) as resultado FROM exportacion WHERE exportacion.cpaides = '".$paiscode."' $queryu $ranfecha GROUP BY exportacion.fano,mes ORDER BY exportacion.fano,mes";
  }
  if($onevar=="ndcl"){
  $query1 = "SELECT exportacion.fano, extract(MONTH from exportacion.fnum) as mes,COUNT(DISTINCT exportacion.ndcl) as resultado FROM exportacion WHERE exportacion.cpaides = '".$paiscode."' $queryu $ranfecha GROUP BY exportacion.fano,mes ORDER BY exportacion.fano,mes";
  }
  if($onevar=="dnombre"){
  $query1 = "SELECT exportacion.fano, extract(MONTH from exportacion.fnum) as mes,COUNT(DISTINCT exportacion.dnombre) as resultado FROM exportacion WHERE exportacion.cpaides = '".$paiscode."' $queryu $ranfecha GROUP BY exportacion.fano,mes ORDER BY exportacion.fano,mes";
  }
  if($onevar=="cpaides"){
  $query1 = "SELECT exportacion.fano, extract(MONTH from exportacion.fnum) as mes,COUNT(DISTINCT exportacion.cpaides) as resultado FROM exportacion WHERE exportacion.cpaides = '".$paiscode."' $queryu $ranfecha GROUP BY exportacion.fano,mes ORDER BY exportacion.fano,mes";
  }
  if($onevar=="cpuedes"){
  $query1 = "SELECT exportacion.fano, extract(MONTH from exportacion.fnum) as mes,COUNT(DISTINCT exportacion.cpuedes) as resultado FROM exportacion WHERE exportacion.cpaides = '".$paiscode."' $queryu $ranfecha GROUP BY exportacion.fano,mes ORDER BY exportacion.fano,mes";
  }
  if($onevar=="cadu"){
  $query1 = "SELECT exportacion.fano, extract(MONTH from exportacion.fnum) as mes,COUNT(DISTINCT exportacion.cadu) as resultado FROM exportacion WHERE exportacion.cpaides = '".$paiscode."' $queryu $ranfecha GROUP BY exportacion.fano,mes ORDER BY exportacion.fano,mes";
  }
   if($onevar=="depa"){
  $query1 = "SELECT exportacion.fano, extract(MONTH from exportacion.fnum) as mes,COUNT(DISTINCT departamento.nombre) as resultado FROM exportacion LEFT JOIN departamento ON ((((SUBSTRING(exportacion.ubigeo,1,2))= departamento.iddepartamento))) WHERE  exportacion.cpaides = '".$paiscode."' $queryu $ranfecha GROUP BY exportacion.fano,mes order BY exportacion.fano, resultado ASC";
  }
  if($onevar=="provi"){
  $query1 = "SELECT exportacion.fano , extract(MONTH from exportacion.fnum) as mes, count(DISTINCT provincia.nombre) as resultado FROM exportacion LEFT JOIN departamento ON  (((SUBSTRING(exportacion.ubigeo,1,2))= departamento.iddepartamento))  INNER JOIN provincia ON provincia.iddepartamento = departamento.iddepartamento  WHERE  exportacion.cpaides =  '".$paiscode."' $queryu $ranfecha GROUP BY exportacion.fano,mes ORDER BY exportacion.fano ASC";
  }
  
  if($onevar=="distri"){
  $query1 = "SELECT exportacion.fano, extract(MONTH from exportacion.fnum) as mes,COUNT(DISTINCT distrito.nombre) as resultado FROM exportacion LEFT JOIN departamento ON ((((SUBSTRING(exportacion.ubigeo,1,2))= departamento.iddepartamento))) INNER JOIN provincia ON provincia.iddepartamento = departamento.iddepartamento  INNER JOIN distrito ON distrito.idprovincia = provincia.idprovincia WHERE exportacion.cpaides = '".$paiscode."' $queryu $ranfecha GROUP BY exportacion.fano,mes ORDER BY exportacion.fano ASC ";
  }
   if($onevar=="cage"){
  $query1 = "SELECT exportacion.fano, extract(MONTH from exportacion.fnum) as mes,COUNT(DISTINCT exportacion.cage) as resultado FROM exportacion WHERE  exportacion.cpaides = '".$paiscode."' $queryu $ranfecha GROUP BY exportacion.fano,mes ORDER BY exportacion.fano,mes";
  }
  if($onevar=="cviatra"){
  $query1 = "SELECT exportacion.fano, extract(MONTH from exportacion.fnum) as mes,COUNT(DISTINCT exportacion.cviatra) as resultado FROM exportacion WHERE exportacion.cpaides = '".$paiscode."' $queryu $ranfecha GROUP BY exportacion.fano,mes ORDER BY exportacion.fano,mes";
  }
...
if($onevar=="diferen"){//precio general total
	  //sumamos valorfob
		   ...
		   FROM exportacion WHERE $ranfechados $queryu AND exportacion.cpaides ='".$paiscode."'";	
...
while($fila_vpes=$resultado_vpes->fetch_array()){
	...
	$vpes_2022= "0.00";
	  ...
	  $query_tota = "SELECT Count(DISTINCT exportacion.part_nandi) as cant_total, extract(year from exportacion.fnum) as anio FROM exportacion WHERE $ranfechados $queryu AND exportacion.cpaides ='".$paiscode."' GROUP BY anio order BY anio ASC";
	    }//fin consulta conteo unico total partida
	 if($onevar=="ndcl"){//duas
	$query_tota = "SELECT Count(DISTINCT exportacion.ndcl) as cant_total, extract(year from exportacion.fnum) as anio FROM exportacion WHERE $ranfechados $queryu AND exportacion.cpaides ='".$paiscode."' GROUP BY anio order BY anio ASC";
	 }
	 if($onevar=="dnombre"){//empresas
	 $query_tota = "SELECT Count(DISTINCT exportacion.dnombre) as cant_total, extract(year from exportacion.fnum) as anio FROM exportacion WHERE $ranfechados $queryu AND exportacion.cpaides ='".$paiscode."' GROUP BY anio order BY anio ASC";
	 }
	 if($onevar=="cpuedes"){//puertos
	  $query_tota = "SELECT Count(DISTINCT exportacion.cpuedes) as cant_total, extract(year from exportacion.fnum) as anio FROM exportacion WHERE $ranfechados $queryu AND exportacion.cpaides ='".$paiscode."' GROUP BY anio order BY anio ASC";
	 }
	 if($onevar=="cadu"){//aduanas
	 $query_tota = "SELECT Count(DISTINCT exportacion.cadu) as cant_total, extract(year from exportacion.fnum) as anio FROM exportacion WHERE $ranfechados $queryu AND exportacion.cpaides ='".$paiscode."' GROUP BY anio order BY anio ASC";
	 }
	 if($onevar=="depa"){//departamento
	 $query_tota = "SELECT COUNT(DISTINCT departamento.nombre) as cant_total, extract(year from exportacion.fnum) as anio FROM exportacion LEFT JOIN departamento ON  ((((SUBSTRING(exportacion.ubigeo,1,2))= departamento.iddepartamento))) WHERE $ranfechados $queryu AND exportacion.cpaides='".$paiscode."' GROUP BY anio ORDER BY anio ASC";
	 }
	 if($onevar=="provi"){//provincia
     $query_tota = "SELECT count(DISTINCT provincia.nombre) as cant_total, extract(year from exportacion.fnum) as anio FROM exportacion LEFT JOIN departamento ON  (((SUBSTRING(exportacion.ubigeo,1,2))= departamento.iddepartamento))  INNER JOIN provincia ON provincia.iddepartamento = departamento.iddepartamento WHERE $ranfechados $queryu AND exportacion.cpaides='".$paiscode."' GROUP BY anio ORDER BY anio ASC";
	 }
	 if($onevar=="distri"){//distrito
	  $query_tota = "SELECT count(DISTINCT distrito.nombre) AS cant_total, extract(year from exportacion.fnum) AS anio FROM exportacion LEFT JOIN distrito ON exportacion.ubigeo= distrito.iddistrito WHERE $ranfechados $queryu AND exportacion.cpaides = '".$paiscode."' GROUP BY anio ORDER BY anio ASC";
	 }
	 if($onevar=="cage"){//agentes
	 $query_tota = "SELECT Count(DISTINCT exportacion.cage) as cant_total, extract(year from exportacion.fnum) as anio FROM exportacion WHERE $ranfechados $queryu AND exportacion.cpaides ='".$paiscode."' GROUP BY anio order BY anio ASC";
	 }
	 if($onevar=="cviatra"){//via transportes
	  $query_tota = "SELECT Count(DISTINCT exportacion.cviatra) as cant_total, extract(year from exportacion.fnum) as anio FROM exportacion WHERE $ranfechados $queryu AND exportacion.cpaides ='".$paiscode."' GROUP BY anio order BY anio ASC";
	 }
...
<div class="col-lg-12 col-12">
					  <div class="box">
						<div class="box-header with-border">
						  <h4 class="box-title">Reporte Mercado Evoluci&oacute;n Anual de Exportaciones</h4>
						</div>
							<div class="box-body">
								<div class="form-group">
								  <label class="form-label"><b>Mercado:</b> <?=$paisname;?> <b>│ Departamento:</b>  <?=$namedepa1;?> <b>│ Variable:</b> <?=$combo;?> <b>│</b> Fecha Numeraci&oacute;n <b>│ Periodo: </b> 2012 - 2022 </label>
								</div>
