...
$paisname = trim($_POST["namepaisA"]);
$paiscode = trim($_POST["codepaisA"]);
...
//sumamos valorfob
		   ...
		   FROM exportacion WHERE $ranfecha $queryx AND exportacion.cpaides ='".$paiscode."'";
...
//cuenta cantidad de partidas
	  $query_parti = "SELECT Count(DISTINCT exportacion.part_nandi) as cant_parti, extract(year from exportacion.fnum) as anio FROM exportacion WHERE $ranfecha $queryx AND exportacion.cpaides ='".$paiscode."' GROUP BY anio order BY anio ASC";
...
//cuenta cantidad de duas
	   $query_dua = "SELECT Count(DISTINCT exportacion.ndcl) as cant_dua, extract(year from exportacion.fnum) as anio FROM exportacion WHERE FROM exportacion WHERE $ranfecha $queryx AND exportacion.cpaides ='".$paiscode."' GROUP BY anio order BY anio ASC";	
...
//contamos cantidad de puertos
	   $query_pue = "SELECT Count(DISTINCT exportacion.cpuedes) as cant_pue, extract(year from exportacion.fnum) as anio FROM exportacion WHERE $ranfecha $queryx AND exportacion.cpaides ='".$paiscode."' GROUP BY anio order BY anio ASC";
...
//contamos cantidad de aduanas
	   $query_adua = "SELECT Count(DISTINCT exportacion.cadu) as cant_adua, extract(year from exportacion.fnum) as anio FROM exportacion WHERE $ranfecha $queryx AND exportacion.cpaides ='".$paiscode."' GROUP BY anio order BY anio ASC";
...
//contamnos cantidad de departamento
	   $query_depa = "SELECT COUNT(DISTINCT departamento.nombre) as departamento, extract(year from exportacion.fnum) as anio FROM exportacion LEFT JOIN departamento ON  ((((SUBSTRING(exportacion.ubigeo,1,2))= departamento.iddepartamento))) WHERE $ranfecha $queryx AND exportacion.cpaides='".$paiscode."' GROUP BY anio ORDER BY anio ASC";	
...
foreach($dpropos as $fila_prov) {
	...
	$rprov2022= "0.00";
	  ...
	  //contamnos cantidad de distrito
	   $query_dist = "SELECT count(DISTINCT distrito.nombre) AS distrito, extract(year from exportacion.fnum) AS anio FROM exportacion LEFT JOIN distrito ON exportacion.ubigeo= distrito.iddistrito WHERE $ranfecha $queryx AND exportacion.cpaides = '".$paiscode."' GROUP BY anio ORDER BY anio ASC";
...
//contamnos cantidad de agente
	   $query_agen = "SELECT Count(DISTINCT exportacion.cage) as cant_agen, extract(year from exportacion.fnum) as anio FROM exportacion WHERE $ranfecha $queryx AND exportacion.cpaides ='".$paiscode."' GROUP BY anio order BY anio ASC";	
...
//contamos cantidad via de transporte
	   $query_via = "SELECT Count(DISTINCT exportacion.cviatra) as cant_via, extract(year from exportacion.fnum) as anio FROM exportacion WHERE $ranfecha $queryx AND exportacion.cpaides ='".$paiscode."' GROUP BY anio order BY anio ASC";	
...
<div class="col-lg-12 col-12">
					  <div class="box">
						<div class="box-header with-border">
						  <h4 class="box-title">Reporte An&aacute;lisis Mercado Indicador Anual - Exportaciones</h4>
						</div>
							<div class="box-body">
								<div class="form-group">
								  <label class="form-label">Mercado: <?=$paisname;?> │ Departamento: <?=$namedepa1;?> │ Fecha Numeraci&oacute;n │ Periodo 2012 - 2022 </label>
...
echo "<td>".number_format($creg_2018,0)."</td>";
	...
	echo "<td>".number_format($creg_2021,0)."</td>";
							echo "<td>".number_format($creg_2022,0)."</td>";
echo "<td>$var13 %</td>";
echo "<td>$var14 %</td>";
echo "</tr>";
echo "<tr>";
