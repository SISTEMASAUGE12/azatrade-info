...
Sum(CASE extract(year from exportacion.fnum) WHEN '2022' THEN ".$nombres2." ELSE 0 END) AS a2022
FROM exportacion WHERE exportacion.cpaides = '".$paiscode."' $ranfecha $queryu ";
  }
  if($nombres2=="diferen"){
	  $sqlyear="SELECT
...
Sum(CASE extract(year from exportacion.fnum) WHEN '2022' THEN 1 ELSE 0 END) AS a2022
FROM exportacion WHERE exportacion.cpaides = '".$paiscode."' $ranfecha $queryu ";
  }
...
COUNT(DISTINCT CASE extract(year from exportacion.fnum) WHEN '2022' THEN exportacion.".$nombres2." ELSE NULL END) AS a2022
FROM exportacion WHERE exportacion.cpaides = '".$paiscode."' $ranfecha $queryu "; 
  }
...
COUNT(DISTINCT CASE extract(year from exportacion.fnum) WHEN '2022' THEN ".$varia." ELSE NULL END) AS a2022
FROM exportacion WHERE exportacion.cpaides = '".$paiscode."' $ranfecha $queryu ";  
  }
	
$result_year = $conexpg -> prepare($sqlyear); 
$result_year -> execute(); 
$taps = $result_year -> fetchAll(PDO::FETCH_OBJ); 
if($result_year -> rowCount() > 0)   { 
foreach($taps as $fila_year) {
...
Sum(CASE extract(year from exportacion.fnum) WHEN '2022' THEN ".$nombres2." ELSE 0 END) AS a2022
FROM exportacion WHERE exportacion.cpaides = '".$paiscode."' $ranfecha $queryu
GROUP BY exportacion.part_nandi ";
...
Sum(CASE extract(year from exportacion.fnum) WHEN '2022' THEN exportacion.vfobserdol ELSE 0 END) AS x2022,
Sum(CASE extract(year from exportacion.fnum) WHEN '2022' THEN exportacion.vpesnet ELSE 0 END) AS xx2022
FROM exportacion WHERE exportacion.cpaides = '".$paiscode."' $ranfecha $queryu GROUP BY exportacion.part_nandi";
...
COUNT(distinct CASE extract(year from exportacion.fnum) WHEN '2021' THEN  exportacion.".$nombres2." ELSE NULL END) AS a2021,
COUNT(distinct CASE extract(year from exportacion.fnum) WHEN '2022' THEN  exportacion.".$nombres2." ELSE NULL END) AS a2022 
FROM exportacion WHERE exportacion.cpaides = '".$paiscode."' $ranfecha $queryu GROUP BY exportacion.part_nandi/*,arancel.descripcion*/";
  }else{
	  if($nombres2=="depa"){
...
COUNT(distinct CASE extract(year from exportacion.fnum) WHEN '2022' THEN  ".$variaA." ELSE NULL END) AS a2022
FROM exportacion WHERE exportacion.cpaides = '".$paiscode."' $ranfecha $queryu GROUP BY exportacion.part_nandi ";
  }
?>
				  <div class="col-lg-12 col-12">
					  <div class="box">
						<div class="box-header with-border">
						  <h4 class="box-title">Reporte Mercado Evoluci&oacute;n Mensual de Exportaciones</h4>
						</div>
							<div class="box-body">
								<div class="form-group">
								  <label class="form-label"><b>Mercado:</b> <?=$paisname;?> <b>│ Departamento:</b> <?=$namedepa1;?> <b>│ Indicador #1:</b> <?=$combo;?> <b>- Variable #2:</b> <?=$combo2;?> <b>│</b> Fecha Numeracion <b>│ Periodo:</b> 2012 - 2022 </label>
								</div>
...
$weps = $resultado_rpt -> fetchAll(PDO::FETCH_OBJ); 
if($resultado_rpt -> rowCount() > 0)   { 
foreach($weps as $fila_rpt) {
	$cuenta = $cuenta + 1;
...
$queryu = $conexpg -> prepare($sqlu); 
$queryu -> execute(); 
$keps = $queryu -> fetchAll(PDO::FETCH_OBJ); 
