			...
			<div class="col-lg-12 col-12">
					  <div class="box">
						...
						<form class="form" method="post">
							<div class="box-body">
								<h4 class="box-title text-info mb-0"><i class="ti-search me-15"></i> Seleccione Departamento y un Sector </h4>
								<hr class="my-15">
								<div class="row">
								  <div class="col-md-3">
...
foreach($djfros as $filavc) {
	...
	echo "<option value=''>Sin Datos</option>";
								  ...
								  <div class="col-md-3">
									<div class="form-group">
									  <label class="form-label">Seleccionar Sector <small class="text-danger"><b>(*)</b></small> </label>
									  <select id="sect" name="sect" class="form-select">
									  <option value="">Seleccionar</option>
									  <?php
$sqlmer="SELECT concat(tipo,sector) as codsec, concat(sector,' - ',tipo) as sector FROM sector GROUP BY tipo, sector ORDER BY sector ASC";
$remv = $conexpg -> prepare($sqlmer); 
$remv -> execute(); 
$mros = $remv -> fetchAll(PDO::FETCH_OBJ); 
if($remv -> rowCount() > 0)   { 
foreach($mros as $filamer) {
	$nivt1 = $filamer -> codsec;
	$nivt2 = $filamer -> sector;
			echo "<option value='$nivt1'>$nivt2</option>";
}
}else{
	echo "<option value=''>Sin Datos</option>";
								  ...
								  <i data-feather="box"></i> Resumen</button>
									</div>
								  </div>
								</div>
							</div>
