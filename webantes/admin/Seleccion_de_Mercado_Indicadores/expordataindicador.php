...
<h3>Codigo Generado: 0000$idreg </h3><br> <em>Guarde Su Codigo para Futuras Consultas</em>
</td>";
echo "</tr>";
echo "<tr>";
...
if (mysql_num_rows($rsnxfr) > 0){
	...
	while($rowr = mysql_fetch_array($rsnr)){
		...
		//seleccionamos datos del pais seleccionado
		$sqlpaiz="SELECT
merca_temp_prod.id_prod,
merca_temp_prod.nom_p,
merca_temp_indi.cod_indicadores,
merca_indicadores.nom_indicador,
merca_indicadores.fuente_nota,
merca_indicadores.fuente_org,
merca_temp_pais.cod_pais,
merca_pais.pais,
merca_tema.tema
FROM
merca_temp_prod
INNER JOIN merca_temp_indi ON merca_temp_prod.id_prod = merca_temp_indi.cod_prod
INNER JOIN merca_indicadores ON merca_temp_indi.cod_indicadores = merca_indicadores.id
...
WHERE
merca_temp_prod.id_prod = '$idreg' AND
merca_temp_pais.cod_pais = '$sector'";
		$rsnz = mysql_query($sqlpaiz);
if (mysql_num_rows($rsnz) > 0){
	
	//mostramos el pais elegido
	$sqlpaa="SELECT
ttp.id_prod,
ttp.nom_p,
tp.cod_pais,
...
FROM
merca_temp_prod AS ttp
INNER JOIN merca_temp_pais AS tp ON ttp.id_prod = tp.cod_prod
INNER JOIN merca_pais AS p ON tp.cod_pais = p.cod_pais
WHERE
ttp.id_prod = '$idreg' AND
tp.cod_pais = '$sector'";
$rsnpaa = mysql_query($sqlpaa);
if (mysql_num_rows($rsnpaa) > 0){
	while($rowpaa = mysql_fetch_array($rsnpaa)){
		$paa1=$rowpaa["pais"];
	...
	echo "<tr>";
	echo "<td colspan='3' align='center' bgcolor='#D8D8D8'>Pais Seleccionado: <b>$paa1</b></td>";
	echo "</tr>";
	echo "<tr>";
	//echo "<td align='center' bgcolor='#F1F1F1'>N°</td>";
	//echo "<td>Porcentaje</td>";
	echo "<td align='center' bgcolor='#F1F1F1'>Tema</td>";
	//echo "<td align='center' bgcolor='#F1F1F1'>Indicador</td>";
	echo "<td align='center' bgcolor='#F1F1F1'>Descripcion</td>";
	echo "<td align='center' bgcolor='#F1F1F1'>Fuente</td>";
		echo "</tr>";
	while($rowz= mysql_fetch_array($rsnz)){
		$items=$items+1;
		$temz =$rowz["tema"];
		$indz =$rowz["nom_indicador"];
		$fuentez =$rowz["fuente_nota"];
		$orgz =$rowz["fuente_org"];
		
		$inddes = $indz.' '.$fuentez;
		
		echo "<tr>";
		//echo "<td>$items</td>";
		echo "<td align='center'>$temz</td>";
		echo "<td align='justify'><b><u>$indz :</u></b> $fuentez</td>";
		//echo "<td>$fuentez</td>";
		echo "<td align='justify'>$orgz</td>";
		echo "</tr>";
		}
		echo "</table>";
		}
