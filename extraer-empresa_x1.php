...
if(!isset($_POST['searchTerm'])){
   ...
   //Mostrar resultados
	$stmt = $conexpg->prepare("SELECT CONCAT(p.razon_social,p.ruc) as DNOMBRE1A, CONCAT(p.razon_social,' - ',p.ruc) as DNOMBRE2 FROM padron_reducido_ruc AS p WHERE CONCAT(p.ruc,p.razon_social) LIKE :DNOMBRE1A  LIMIT :limit");
