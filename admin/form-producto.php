        ...
        <?php if($ale=="yes"){ ?>
		  ...
		  <?php if($ale=="ok"){ ?>
           ...
           <div class="alert alert-warning">
            ...
            <span>
              <b> Mensaje - </b> Producto ya existe (numero de partida).</span>
          </div>
           <?php } ?>
                                            ...
                                            <div class="col-md-2">
                                                <div class="form-group">
                                                   <label for="exampleEmail" class="bmd-label-floating">Numero de Partida</label>
                                                    <input type="text" name="numpartida" class="form-control"  value="<?php echo "$datonomp1"; ?>" autofocus <?=$rreeqqu;?> <?=$actinput;?> />
      ...
      <option value="">Seleccione Subsector</option>
                                           ...
                                           <div class="col-md-12">
											...
											<div class="col-md-4">
												...
												<?php }else{ ?>
												<select name='tipoprodu' class="select-css" id="tipoprodu"  required>";
														<option value=''>Selec. Tipo Producto</option>
													<option value="Expo">Exportaci&oacute;n</option>
													<option value="Impo">Importaci&oacute;n</option>
												</select>
												<?php } ?>
                                            </div>
                                            ...
                                            <button type="submit" name="register" class="btn btn-fill btn-success">Actualizar <i class="material-icons">save</i></button>
												<?php }else{ ?>
												<button type="submit" name="grabar" class="btn btn-fill btn-success">Registrar <i class="material-icons">save</i></button>
												<?php } ?>
...
if(isset($_POST['grabar'])){ 
	$kidt = trim($_POST["numpartida"]);
	
	//consultamos si la partida ya existe
	$sqlnodo2 = "SELECT partida_nandi FROM productos WHERE partida_nandi='$kidt' AND estado='A' ";
	$rsndo2=$conexpg->query($sqlnodo2); 
if($rsndo2->num_rows>0){ 
while($finodo2=$rsndo2->fetch_array()){
             $exipart = $finodo2['partida_nandi'];
}
}else{
	$exipart = "";
}
...
'".$_POST["vistados"]."')";
   ...
   echo"<script>location.href='form-producto.php?if=$ediale'</script>";
